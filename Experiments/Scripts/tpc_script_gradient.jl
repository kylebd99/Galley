...
function finch_sp_lr(li_order, orders_x, order_cust, customer_x, li_supp, supplier_x, li_part, part_x, Y, θ)
...
f_ls = []
for i in 1:2
    f_time, d = finch_sp_lr(li_order, orders_x, order_cust, customer_x, li_supp, supplier_x, li_part, part_x, lineitem_y, θ)
    push!(f_times, f_time)
    push!(f_ls, d)
end
...
function finch_sp_log(li_order, orders_x, order_cust, customer_x, li_supp, supplier_x, li_part, part_x, Y, θ)
    X = Tensor(Dense(Dense(Element(0.0))))
...
f_logs = []
for i in 1:2
    f_time, d = finch_sp_log(li_order, orders_x, order_cust, customer_x, li_supp, supplier_x, li_part, part_x, lineitem_y, θ)
...
function finch_sp_log2(li_supp, supplier_x, li_part, part_x, li_y, θ)
    ...
    println("nnz(prediction): $(countstored(prediction))")
    println("size(prediction): $(size(prediction))")
    f_time += @elapsed @finch begin
        d .= 0
        for i1=_
            for i2=_
...
sigmoid(x) = exp(x-log1pexp(x))
exp_term = Materialize(t_undef, t_undef, :i1, :i2, MapJoin(sigmoid, Σ(:k, MapJoin(*, X_g[:i1, :i2, :k], Input(θ, :k)))))
d_g = Query(:d_g, Materialize(t_undef, :j, Σ(:i1, :i2, MapJoin(*, X_g[:i1, :i2, :j], MapJoin(+, Y[:i1, :i2], MapJoin(-, exp_term[:i1, :i2]))))))
g_log_times = []
g_log_opt_times = []
for i in 1:2
    result_galley = galley(d_g, ST=DCStats,  verbose=0)
...
println("F = G: $(sum(abs.(f_logs[1] - result_log.value)))")
