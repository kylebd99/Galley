...
MapJoin(op, inputs...) = LogicalPlanNode(MapJoin, [op, inputs...], nothing)
Reorder(input, index_order) = LogicalPlanNode(Reorder, [input, index_order], nothing)
InputTensor(tensor::Tensor)  = LogicalPlanNode(InputTensor, [IndexExpr[], tensor], NaiveStats)
InputTensor(tensor::Tensor, stats_type::Type)  = LogicalPlanNode(InputTensor, [IndexExpr[], tensor], stats_type)
