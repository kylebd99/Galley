...
function test_fill()
    ...
    X_instance = X_access
    output_tensor = Tensor(SparseHashLevel{1}(SparseHashLevel{1}(Element(0.0))))
    output_tag = tag_instance(variable_instance(:output_tensor), output_tensor)
    ...
    initializer = declare_instance(tag_instance(variable_instance(:output_tensor), output_tensor), literal_instance(0.0))
    ...
    println("Output Size: ", countstored(output_tensor))
