...
n = 10000
k = 100
A = Tensor(Dense(SparseList(Element(0))), fsprand(Int, n, n, .0001) .% 2)
W = Tensor(Dense(SparseList(Element(0))), fsprand(Int, k, k, .1)  .% 10)
...
h_3_galley = galley(deepcopy(two_hop_gnn_query), ST=NaiveStats, verbose=0)
h_3_galley = galley(deepcopy(two_hop_gnn_query), ST=NaiveStats, verbose=0)
