...
println("Running SparseByteMap Version: $matrix")
sparsebytemap_p4 = nothing
f_sparsebytemap_times = []
for i in  1:3
    p1 = Tensor(SparseByteMap(Element(false)), p)
    q1 = Tensor(SparseByteMap(Element(false)), q)
    p2 = Tensor(SparseByteMap(Element(false)))
    q2 = Tensor(SparseByteMap(Element(false)))
    p3 = Tensor(SparseByteMap(Element(false)))
    q3 = Tensor(SparseByteMap(Element(false)))
    p4 = Tensor(SparseByteMap(Element(false)))
    q4 = Tensor(SparseByteMap(Element(false)))
    rounds = 0
    f_sparsebytemap_time = 0
    while  (rounds == 0 || sum(q4) > 0) && rounds < n_rounds
        f_sparsebytemap_time += @elapsed @finch begin
            q2 .= false
            for n2=_
                for n1=_
        ...
        f_sparsebytemap_time += @elapsed p1 = Tensor(SparseByteMap(Element(false)), p4)
        f_sparsebytemap_time += @elapsed q1 = Tensor(SparseByteMap(Element(false)), q4)
        rounds += 1
    end
    push!(f_sparsebytemap_times, f_sparsebytemap_time)
    global sparsebytemap_p4 = p4
end
...
for i in 1:3
    ...
    while (rounds == 0 || sum(q) > 0) && rounds < n_rounds
        ...
        global result_galley = galley(query, simple_cse=false, faq_optimizer=greedy, ST=DCStats, verbose=verbose)
...
for i in 1:3
    ...
    while (rounds == 0 || sum(q) > 0) && rounds < n_rounds
        query = get_bfs_query(A_g, q_g, p_g)
        global result_galley_cse = galley(query, simple_cse=true, faq_optimizer=greedy, ST=DCStats, verbose=verbose)
